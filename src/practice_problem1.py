...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        numbers_of_characters_to_append = min(space, len(additional_contents))
        stuff_to_add = ''
        for k in range(numbers_of_characters_to_append):
            stuff_to_add = stuff_to_add + additional_contents[k]
        self.contents = self.contents + stuff_to_add
        stuff_to_return = ''
        for k in range(numbers_of_characters_to_append, len(additional_contents)):
            stuff_to_return += additional_contents[k]
    ...
    def shrink(self, new_volume):
        ...
        for k in range(self.volume, len(self.contents)):
            stuff_left += self.contents[k]
        if self.volume < len(self.contents):
            new_stuff = ''
            for k in range(self.volume):
                new_stuff += self.contents[k]
            self.contents = new_stuff
        return stuff_left
    ...
    def double_then_shrink(self, new_volume):
        ...
        first = self.double()
        self.volume = new_volume
        second = self.shrink(self.volume)
        self.numbers_discarded = len(first) + len(second)
        return self.numbers_discarded
    ...
    def steal(self, other_box):
        ...
        #    ** TWO **   LINES OF CODE.
        ################################################################
        yay = self.append_string(other_box.contents)
        other_box.contents = yay
    ...
    def combined_box(self, other_box):
        ...
        volume = len(self.contents) + len(other_box.contents)
